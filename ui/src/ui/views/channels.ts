...
function renderGenericChannelCard(
  ...
  return html`
    <div class="card">
      ...
      <div class="card-sub">${t().channels.status.configured} & ${t().common.status}</div>
      ${accountCountLabel}
          ...
          : html`
            <div class="status-list" style="margin-top: 16px;">
              <div>
                ...
                <span>${configured == null ? t().status.na : configured ? t().status.yes : t().status.no}</span>
              </div>
              <div>
                <span class="label">${t().channelsView.running}</span>
                <span>${running == null ? t().status.na : running ? t().status.yes : t().status.no}</span>
              </div>
              <div>
                <span class="label">${t().channelsView.connected}</span>
                <span>${connected == null ? t().status.na : connected ? t().status.yes : t().status.no}</span>
              </div>
            </div>
          `
      }
...
function deriveRunningStatus(account: ChannelAccountSnapshot): string {
  if (account.running) return t().status.yes;
  // If we have recent inbound activity, the channel is effectively running
  if (hasRecentActivity(account)) return t().common.active;
  return t().status.no;
}

function deriveConnectedStatus(account: ChannelAccountSnapshot): string {
  if (account.connected === true) return t().status.yes;
  if (account.connected === false) return t().status.no;
  ...
  return html`
    <div class="account-card">
      ...
      <div class="status-list account-card-status">
        ...
        <div>
          <span class="label">${t().channelsView.configured}</span>
          <span>${account.configured ? t().status.yes : t().status.no}</span>
        </div>
        <div>
          <span class="label">${t().channelsView.connected}</span>
        ...
        <div>
          ...
          <span>${account.lastInboundAt ? formatAgo(account.lastInboundAt) : t().status.na}</span>
        </div>
        ${
          account.lastError
            ? html`
