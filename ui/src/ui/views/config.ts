...
function getSectionLabel(key: string): string {
  const labels = t().config.sections;
  switch (key) {
    case "env":
...
export function renderConfig(props: ConfigProps) {
  const validityStatus = props.valid == null ? "unknown" : props.valid ? "valid" : "invalid";
  const validity =
    props.valid == null
      ? t().common.unknown
      : props.valid
  ...
  const availableSections = SECTION_KEYS.filter((key) => key in schemaProps).map((key) => ({
    key,
  ...
  return html`
    <div class="config-layout">
      <!-- Header bar with search, status, and actions -->
      <header class="config-header">
        <div class="config-header__left">
          <div class="config-search">
            ...
            ${
              props.searchQuery
                ? html`<button class="config-search__clear" @click=${() => props.onSearchChange("")}>×</button>`
                : nothing
            }
          </div>
          <span class="config-status-pill ${validityStatus === "valid" ? "ok" : validityStatus === "invalid" ? "danger" : ""}">${validity}</span>
          ${
            hasChanges
              ? html`<span class="config-changes-pill">${props.formMode === "raw" ? t().common.unsaved : `${diff.length} ${diff.length === 1 ? t().common.change : t().common.changes}`}</span>`
              : nothing
          }
        </div>
        <div class="config-header__right">
          <div class="config-mode-toggle">
            ...
            >${t().config.mode.form}</button>
            <button
              class="config-mode-toggle__btn ${props.formMode === "raw" ? "active" : ""}"
              @click=${() => props.onFormModeChange("raw")}
            >${t().config.mode.raw}</button>
          </div>
          <button class="config-btn" ?disabled=${props.loading} @click=${props.onReload}>
            ${props.loading ? "..." : t().common.reload}
          </button>
          <button class="config-btn primary" ?disabled=${!canSave} @click=${props.onSave}>
            ${props.saving ? "..." : t().common.save}
          </button>
          <button class="config-btn" ?disabled=${!canApply} @click=${props.onApply}>
            ${props.applying ? "..." : t().common.apply}
          </button>
          <button class="config-btn" ?disabled=${!canUpdate} @click=${props.onUpdate}>
            ${props.updating ? "..." : t().common.update}
          </button>
        </div>
      </header>
      ...
      <nav class="config-tabs">
        ...
        >
          <span class="config-tab__icon">${sidebarIcons.all}</span>
          <span class="config-tab__label">${t().config.sections.all}</span>
        </button>
        ${allSections.map(
          (section) => html`
          <button
      ...
      <!-- Subsection pills (when a section is selected) -->
      ${
        allowSubnav
          ? html`
            <div class="config-subtabs">
              <button
                class="config-subtab ${effectiveSubsection === null ? "active" : ""}"
                @click=${() => props.onSubsectionChange(ALL_SUBSECTION)}
              >${t().config.sections.all}</button>
              ${subsections.map(
                (entry) => html`
                  <button
                    class="config-subtab ${effectiveSubsection === entry.key ? "active" : ""}"
        ...
        hasChanges && props.formMode === "form"
          ...
          <details class="config-diff">
            <summary class="config-diff__summary">
              ${t().config.diff.view} ${diff.length} ${diff.length === 1 ? t().config.diff.pendingChange : t().config.diff.pendingChanges}
            ...
            <div class="config-diff__content">
              ${diff.map(
                ...
                <div class="config-diff__item">
                  <span class="config-diff__path">${change.path}</span>
        ...
        ${
          props.formMode === "form"
            ? html`
              ...
              ${
                formUnsafe
                  ? html`<div class="callout danger" style="margin-top: 12px">${t().config.form.rawWarning}</div>`
                  : nothing
              }
            `
            : html`
              <label class="field config-raw-field">
                <span>${t().config.form.rawJson}</span>
                <textarea
                  .value=${props.raw}
      ...
      ${
        props.issues.length > 0
          ? html`<div class="callout danger" style="margin: 0 16px 16px;"><pre class="code-block">${JSON.stringify(props.issues, null, 2)}</pre></div>`
          : nothing
      }
    </div>
  `;
