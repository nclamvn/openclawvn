...
export function renderNostrCard(params: {
    ...
    return html`
      <div class="account-card">
        ...
        <div class="status-list account-card-status">
          ...
          <div>
            <span class="label">${t().channelsView.pubkey}</span>
    ...
    return html`
      <div style="margin-top: 16px; padding: 12px; background: var(--bg-secondary); border-radius: 8px;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
          <div style="font-weight: 500;">${t().channelsView.profile}</div>
          ${
            summaryConfigured
              ? html`
                <button
          ...
          hasAnyProfileData
            ? html`
              <div class="status-list">
                ...
                ${
                  displayName
                    ? html`<div><span class="label">${t().channelsView.displayNameLabel}</span><span>${displayName}</span></div>`
                    : nothing
                }
                ${
                  about
  ...
  return html`
    <div class="card">
      ...
      <div class="card-sub">${t().channelsView.nostrCardDesc}</div>
      ${accountCountLabel}
