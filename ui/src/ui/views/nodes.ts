    ...
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          ...
          <div class="card-sub">${t().nodes.nodes.description}</div>
        </div>
        <button class="btn" ?disabled=${props.loading} @click=${props.onRefresh}>
          ${props.loading ? t().nodesView.loading : t().nodesView.refresh}
        </button>
      </div>
      <div class="list" style="margin-top: 16px;">
        ${
          props.nodes.length === 0
            ? html`
                <div class="muted">${t().nodes.nodes.empty}</div>
              `
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          ...
          <div class="card-sub">${t().nodes.devices.description}</div>
        </div>
        <button class="btn" ?disabled=${props.devicesLoading} @click=${props.onDevicesRefresh}>
          ${props.devicesLoading ? t().nodesView.loading : t().nodesView.refresh}
        </button>
      </div>
      ${
        props.devicesError
    ...
    <div class="list-item">
      <div class="list-main">
        ...
        ${
          tokens.length === 0
            ? html`
                <div class="muted" style="margin-top: 6px">${t().nodes.deviceDetails.tokensNone}</div>
              `
            : html`
              <div class="muted" style="margin-top: 10px;">${t().nodes.deviceDetails.tokens}</div>
...
function getSecurityOptions(): Array<{ value: ExecSecurity; label: string }> {
  return [
    { value: "deny", label: t().nodesView.security.deny },
    { value: "allowlist", label: t().nodesView.security.allowlist },
    { value: "full", label: t().nodesView.security.full },
  ];
}

function getAskOptions(): Array<{ value: ExecAsk; label: string }> {
  return [
    { value: "off", label: t().nodesView.ask.off },
    { value: "on-miss", label: t().nodesView.ask.onMiss },
    { value: "always", label: t().nodesView.ask.always },
  ];
}
...
function renderBindings(state: BindingState) {
  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between; align-items: center;">
        <div>
          <div class="card-title">${t().nodes.execBinding.title}</div>
          <div class="card-sub">
            ${t().nodes.execBinding.description} <span class="mono">exec host=node</span>.
          </div>
        </div>
        <button
          class="btn"
          ...
          @click=${state.onSave}
        >
          ${state.configSaving ? t().nodesView.saving : t().nodesView.save}
        </button>
      </div>
        ...
        state.formMode === "raw"
          ? html`
              <div class="callout warn" style="margin-top: 12px">
                ${t().nodes.execBinding.switchToForm} <strong>${t().nodes.execBinding.formMode}</strong> ${t().nodes.execBinding.toEditBindings}
              </div>
            `
          : nothing
      }
      ...
      ${
        !state.ready
          ? html`<div class="row" style="margin-top: 12px; gap: 12px;">
            <div class="muted">${t().nodes.execBinding.loadConfig}</div>
            <button class="btn" ?disabled=${state.configLoading} @click=${state.onLoadConfig}>
              ${state.configLoading ? t().nodesView.loading : t().nodesView.loadConfig}
            </button>
          </div>`
          : html`
            <div class="list" style="margin-top: 16px;">
              <div class="list-item">
                <div class="list-main">
                  <div class="list-title">${t().nodes.execBinding.defaultBinding}</div>
                  <div class="list-sub">${t().nodes.execBinding.defaultBindingDesc}</div>
                </div>
                <div class="list-meta">
                  <label class="field">
                    <span>${t().nodes.execBinding.node}</span>
                    <select
                    ...
                    >
                      <option value="" ?selected=${defaultValue === ""}>${t().nodes.execBinding.anyNode}</option>
                      ${state.nodes.map(
                        (node) =>
                          html`<option
                            value=${node.id}
                  ...
                  ${
                    !supportsBinding
                      ? html`
                          <div class="muted">${t().nodes.execBinding.noNodesAvailable}</div>
                        `
                      : nothing
                  }
                </div>
              ...
              ${
                state.agents.length === 0
                  ? html`
                      <div class="muted">${t().nodes.execBinding.noAgents}</div>
                    `
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between; align-items: center;">
        <div>
          ...
          <div class="card-sub">
            ${t().nodes.execApprovals.description} <span class="mono">exec host=gateway/node</span>.
          </div>
        </div>
        <button
          class="btn"
          ...
          @click=${state.onSave}
        >
          ${state.saving ? t().nodesView.saving : t().nodesView.save}
        </button>
      </div>
      ...
      ${
        !ready
          ? html`<div class="row" style="margin-top: 12px; gap: 12px;">
            <div class="muted">${t().nodes.execApprovals.loadApprovals}</div>
            <button class="btn" ?disabled=${state.loading || !targetReady} @click=${state.onLoad}>
              ${state.loading ? t().nodesView.loading : t().nodesView.loadApprovals}
            </button>
          </div>`
          : html`
    ...
    <div class="list" style="margin-top: 12px;">
      <div class="list-item">
        <div class="list-main">
          ...
          <div class="list-sub">
            ${t().nodes.execApprovals.targetDesc}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
            ...
            >
              ...
              <option value="node" ?selected=${state.target === "node"}>${t().nodes.execBinding.node}</option>
            </select>
          </label>
          ${
            state.target === "node"
...
function renderExecApprovalsPolicy(state: ExecApprovalsState) {
  ...
  return html`
    <div class="list" style="margin-top: 16px;">
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${t().nodes.security.title}</div>
          <div class="list-sub">
            ${isDefaults ? t().nodesView.security.defaultMode : `${t().nodesView.security.defaultIs} ${defaults.security}.`}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
            <span>${t().nodes.security.mode}</span>
            <select
              ?disabled=${state.disabled}
              @change=${(event: Event) => {
                const target = event.target as HTMLSelectElement;
              ...
              ${
                !isDefaults
                  ? html`<option value="__default__" ?selected=${securityValue === "__default__"}>
                    ${t().common.useDefault} (${defaults.security})
                  </option>`
                  : nothing
              }
      ...
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${t().nodes.ask.title}</div>
          <div class="list-sub">
            ${isDefaults ? t().nodesView.ask.defaultPolicy : `${t().nodesView.ask.defaultIs} ${defaults.ask}.`}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
              ...
              ${
                !isDefaults
                  ? html`<option value="__default__" ?selected=${askValue === "__default__"}>
                    ${t().common.useDefault} (${defaults.ask})
                  </option>`
                  : nothing
              }
              ${getAskOptions().map(
      ...
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${t().nodes.ask.fallback}</div>
          <div class="list-sub">
            ${
              isDefaults
                ? t().nodesView.ask.fallbackDesc
                : `${t().nodesView.ask.defaultIs} ${defaults.askFallback}.`
            }
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
            <span>${t().nodes.ask.fallback}</span>
            <select
              ?disabled=${state.disabled}
              @change=${(event: Event) => {
                const target = event.target as HTMLSelectElement;
              ...
              ${
                !isDefaults
                  ? html`<option value="__default__" ?selected=${askFallbackValue === "__default__"}>
                    ${t().common.useDefault} (${defaults.askFallback})
                  </option>`
                  : nothing
              }
              ${getSecurityOptions().map(
      ...
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${t().nodes.autoAllowSkills.title}</div>
          <div class="list-sub">
            ${
              isDefaults
                ? t().nodesView.autoAllow.allowSkillExecs
                : autoIsDefault
                  ? `${t().nodesView.autoAllow.usingDefault} (${defaults.autoAllowSkills ? t().nodesView.autoAllow.on : t().nodesView.autoAllow.off}).`
                  : `${t().nodesView.autoAllow.override} (${autoEffective ? t().nodesView.autoAllow.on : t().nodesView.autoAllow.off}).`
            }
          </div>
        </div>
        <div class="list-meta">
  ...
  return html`
    <div class="row" style="margin-top: 18px; justify-content: space-between;">
      <div>
        ...
        <div class="card-sub">${t().nodes.allowlist.description}</div>
      </div>
      <button
        class="btn btn--sm"
        ?disabled=${state.disabled}
    ...
    <div class="list" style="margin-top: 12px;">
      ${
        entries.length === 0
          ? html`
              <div class="muted">${t().nodes.allowlist.empty}</div>
            `
...
function renderAllowlistEntry(
  ...
  const lastUsed = entry.lastUsedAt ? formatAgo(entry.lastUsedAt) : t().nodes.allowlist.never;
  const lastCommand = entry.lastUsedCommand ? clampText(entry.lastUsedCommand, 120) : null;
  const lastPath = entry.lastResolvedPath ? clampText(entry.lastResolvedPath, 120) : null;
  return html`
    <div class="list-item">
      <div class="list-main">
        <div class="list-title">${entry.pattern?.trim() ? entry.pattern : t().nodesView.allowlist.newPattern}</div>
        <div class="list-sub">${t().nodesView.allowlist.lastUsed}${lastUsed}</div>
  ...
  return html`
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="list-sub">
          ${agent.isDefault ? t().nodesView.binding.defaultAgent : t().nodesView.binding.agent} ·
          ${
            bindingValue === "__default__"
              ? `${t().nodesView.binding.usesDefault} (${state.defaultBinding ?? t().nodesView.binding.any})`
              : `${t().nodes.agentBinding.override} ${agent.binding}`
          }
        </div>
      </div>
      <div class="list-meta">
        <label class="field">
          <span>${t().nodes.agentBinding.binding}</span>
          <select
            ?disabled=${state.disabled || !supportsBinding}
            @change=${(event: Event) => {
              const target = event.target as HTMLSelectElement;
...
function renderNode(node: Record<string, unknown>) {
  ...
  const title =
    (typeof node.displayName === "string" && node.displayName.trim()) ||
    (typeof node.nodeId === "string" ? node.nodeId : t().status.unknown);
    ...
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="chip-row" style="margin-top: 6px;">
          <span class="chip">${paired ? t().nodes.nodeStatus.paired : t().nodes.nodeStatus.unpaired}</span>
          <span class="chip ${connected ? "chip-ok" : "chip-warn"}">
            ${connected ? t().nodes.nodeStatus.connected : t().nodes.nodeStatus.offline}
          </span>
