...
// Emoji to Lucide icon mapping for skills
...
export function getIconForEmoji(emoji: string | undefined): TemplateResult | null {
  if (!emoji) return null;
  const iconName = emojiToIconMap[emoji];
  if (iconName) {
    return icons[iconName];
  }
  // Default to puzzle icon for unknown emojis
  return icons.puzzle;
}

export function renderSkillIcon(emoji: string | undefined, className = "skill-icon"): TemplateResult {
  const iconSvg = getIconForEmoji(emoji);
  if (!iconSvg) {
    return html`<span class="${className}" aria-hidden="true">${icons.puzzle}</span>`;
  }
  return html`<span class="${className}" aria-hidden="true">${iconSvg}</span>`;
}
