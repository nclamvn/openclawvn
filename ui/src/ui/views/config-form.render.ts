...
export function renderConfigForm(props: ConfigFormProps) {
  ...
  if (schemaType(schema) !== "object" || !schema.properties) {
    return html`
      <div class="callout danger">${t().config.form.schemaUnsupported}</div>
    `;
  }
    ...
    return html`
      <div class="config-empty">
        ...
        <div class="config-empty__text">
          ${searchQuery ? `${t().config.form.noMatchingSettings} "${searchQuery}"` : t().config.form.noSettingsInSection}
        </div>
      </div>
    `;
  }
